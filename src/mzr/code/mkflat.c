...
int main(int argc,char *argv[])
{
  ...
  double cf[4][4],cfs;
  int i1,i2,j1,j2,i1t,i2t,j1t,j2t,icen;
  ...
  if(fits_read_key(fp, TSTRING, "JST-STR", jststr, NULL, &status)) printerror(status);
  ...
  if(fits_write_key(fp, TSTRING, "JST-STR", jststr, NULL, &status)) printerror(status);
  ...
  if(fits_write_key(fp, TINT, "SPEC-CTR", &icen, NULL, &status)) printerror(status);
  fpixel=1;
  fprintf(stderr,"%ld x %ld format ",naxes[0],naxes[1]);
  for(i=0;i<naxes[1];i++){
