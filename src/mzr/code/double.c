...
float med(double sp[LINEPIX2],int jc){
  float m[MW+1],md;
  int j,k,kk,n;
...
int main(int argc,char *argv[])
{
  ...
  static float zi[LINEPIY][LINEPIX2],zo[LINEPIY2][LINEPIX2],zn[LINEPIY2][LINEPIX2],zx[LINEPIY2][LINEPIX2],zy[LINEPIY2][LINEPIX2],zp[LINEPIY2][LINEPIX2],bn[40][40],rfs[NFIBER][LINEPIX2/48],rf0[NFIBER][LINEPIX2],rf[NFIBER][LINEPIX2],bg0[2][LINEPIX2];
  static double sp[NFIBER][LINEPIX2],bg[NFIBER][LINEPIX2];
  double kn[PSPEC],wc,dw,ax[4][3],ay[4][3],xav[3][LINEPIX2],yav[2][LINEPIY2],fav[3][NFIBER],fav0[NFIBER],kns,bav,bavav,cw[NCAL],dmy,ww[LINEPIX2],aa[MAXODR],ew,ewn;
  ...
  while(1){
    ...
    if(new&&argc>=4){
      ...
      if(fits_read_keys_lng(fp[1], "NAXIS", 1, 2, naxes, &nfound, &status)) printerror(status);
      if(fits_read_key(fp[1], TDOUBLE, "CRVAL1", &wc, NULL, &status)) printerror(status);
      if(fits_read_key(fp[1], TDOUBLE, "CDELT1", &dw, NULL, &status)) printerror(status);
      fprintf(stderr,"icen=%d ",icen);
      if(fits_read_key(fp[1], TINT, "SPEC-CTR", &icen, NULL, &status)){
	fprintf(stderr,"No SPEC-CTR ... ");
	status=0;
      }
      fprintf(stderr,"=> %d ",icen);
	...
	yav[0][i]/=n;
      ...
      for(ii=0;ii<PSPEC;ii++) kn[ii]=0.;
      for(nf=0;nf<NFIBER;nf++){
	i=LINE0+PSPEC*nf-PSPEC/2;
	for(ii=0;ii<PSPEC;ii++) kn[ii]+=yav[0][i+ii];
      }
      ymin=1e10;ymax=-1e10;
      for(ii=0;ii<PSPEC;ii++){
	kn[ii]/=NFIBER;
	//fprintf(stderr,"%d: %g\n",ii,kn[ii]);
	if(ymin>kn[ii]) ymin=kn[ii];
	if(ymax<kn[ii]){ymax=kn[ii];im=ii;}
      }
      //fprintf(stderr,"ymin=%g ymax=%g imax=%d",ymin,ymax,im-PSPEC/2);getchar();
      //for(ii=0;ii<PSPEC;ii++) fprintf(stderr," %g",kn[ii]);
      //fprintf(stderr,"\n");
      sft=0;
      if(ymax+ymin>0.1 && ymax-ymin>10. && (ymax-ymin)/(ymax+ymin)>0.1 && abs(im-PSPEC/2)>=1){
	fprintf(stderr,"Position Shift is detected !\n");
	for(ii=0;ii<PSPEC;ii++) fprintf(stderr," %g",kn[ii]);
	fprintf(stderr,"\n");
	if(abs(im-PSPEC/2)>=2) sft=1;
	if(im-PSPEC/2>0){
	  for(i=im-PSPEC/2;i<naxes[1];i++){
	    for(j=0;j<naxes[0];j++) zo[i-(im-PSPEC/2)][j]=zo[i][j];
	  }
	}
	else{
	  for(i=naxes[1]+im-PSPEC/2-1;i>=0;i--){
	    for(j=0;j<naxes[0];j++) zo[i-(im-PSPEC/2)][j]=zo[i][j];
	  }
	}
      }
	...
	for(ii=0;ii<PSPEC;ii++){
	  for(j=0;j<naxes[0];j++){
	    zo[i+ii][j]/=rf0[nf][j];
	    zn[i+ii][j]/=rf0[nf][j];//*rf0[nf][j] の分は最後に...
	...
	for(j=0;j<naxes[0];j++) zp[i][j]=zo[i][j];
      ...
      for(lp=0;lp<4;lp++){
	for(i=0;i<naxes[1];i++){
	  yav[0][i]=0.;yav[1][i]=0.;n=0;
	  for(j=1;j<naxes[0]-1;j++){
	    if(isnan(zo[i][j-1])==0&&isnan(zo[i][j])==0&&isnan(zo[i][j+1])==0){
	      yav[0][i]+=zo[i][j];
	      if(zo[i][j]>0.) yav[1][i]+=zn[i][j]+zo[i][j];
	      else yav[1][i]+=zn[i][j];
	      n++;
	    }
	  }
	  yav[0][i]/=n;yav[1][i]/=n;
	  //if(yav[1][i]<1e-5) fprintf(stderr,"%d: %g %g %d\n",i,yav[0][i],yav[0][i],n);
	}
	for(ii=0;ii<PSPEC;ii++){
	  kn[ii]=0.;
	  for(nf=0;nf<NFIBER;nf++){
	    i=LINE0+PSPEC*nf-PSPEC/2;
	    kn[ii]+=yav[0][i+ii]/yav[1][i+ii];
	  }
	}
	//for(ii=0;ii<PSPEC;ii++) fprintf(stderr,"%d: %g\n",ii,kn[ii]);
	ymax=-1e10;
	for(ii=0;ii<PSPEC;ii++){
	  if(ymax<kn[ii]) ymax=kn[ii];
	  if(kn[ii]<0.) kn[ii]=0.;
	}
	if(lp==3) fprintf(stderr,"Kernel: ");
	for(ii=0;ii<PSPEC;ii++){
	  if(ymax<1.) kn[ii]=1.;
	  else kn[ii]/=ymax;
	  if(lp==3) fprintf(stderr," %g",kn[ii]);
	}if(lp==3) fprintf(stderr,"\n");
	kns=0.;
	for(ii=0;ii<PSPEC;ii++) kns+=kn[ii];
	favav[0]=0.;favav[1]=0.;bavav=0.;
	favmin=1e99;favmax=-1e99;
	for(nf=0;nf<NFIBER;nf++){
	  i=LINE0+PSPEC*nf-PSPEC/2;
	  fav[0][nf]=0.;fav[2][nf]=0.;bav=0.;
	  for(ii=0;ii<PSPEC;ii++){
	    fav[0][nf]+=yav[0][i+ii]*kn[ii];
	    fav[2][nf]+=yav[1][i+ii]*kn[ii]*kn[ii];
	    bav+=yav[0][i+ii]*(1-kn[ii]);
	  }
	  ...
	  favav[1]+=fav[1][nf];
	  bavav+=bav;
	  //fprintf(stderr,"%d: %g %g %g %g\n",nf,fav[0][nf],fav[1][nf],fav[2][nf],fav0[nf]);
	}//getchar();
	favav[0]/=NFIBER;
	favav[1]/=NFIBER;
	...
	if(favav[1]<10){
	  ...
	  for(nf=0;nf<NFIBER;nf++){
	    fav[2][nf]=fav[0][nf]/fav[2][nf];
	    fav[0][nf]/=favav[0];
	    fav[1][nf]/=favav[1];
	    //fprintf(stderr,"%d: %g %g %g %g\n",nf,fav[0][nf],fav0[nf],fav[1][nf],fav[2][nf]);
	  }
	  if(lp<3){
	    n=0;sk=0.;
	    for(nf=0;nf<NFIBER;nf++){
	...
	for(j=0;j<naxes[0];j++){
	  ...
	  for(nf=0;nf<NFIBER;nf++){
		...
		if(lp==3){
		  ...
		  //if(j==naxes[0]/2){fprintf(stderr,"%d %d %g %g %g %g %g %g",nf,i+ii,zo[i+ii][j],zn[i+ii][j],zo[i+ii][j]/zn[i+ii][j],sp[nf][j],bg[nf][j],sp[nf][j]/bg[nf][j]);getchar();}
		}
	      }
	    }
	    if(wk<1.) sp[nf][j]=0./0.;
	    ...
	    if(lp==3){
	      ...
	      if(fav[0][nf]>1e-5){
		xav[0][j]+=sp[nf][j]*fav[2][nf];
		wt+=fav[2][nf];
		if(isnan(sp[nf][j])==0) xav[2][j]+=bg[nf][j]*fav[2][nf]*fav[2][nf];
		//if(j==naxes[0]/2){fprintf(stderr,"%d %g %g %g %g %g %g",nf,sp[nf][j],bg[nf][j],sp[nf][j]/bg[nf][j]*61,fav[2][nf],xav[0][j],xav[2][j]);getchar();}
	      }
	      if(fav[0][nf]-sk<1e-5){
		  xav[1][j]+=sp[nf][j];
		  ws+=1.;
		...
		if(isnan(sp[nf][j])==0){
		  xav[0][j]+=sp[nf][j]*fav[2][nf];
		  wt+=fav[2][nf];
		  //fprintf(stderr,"%d %d %g %g %g %g",nf,j,xav[0][j],sp[nf][j],rf0[nf][j],fav[2][nf]);getchar();
		}
	      }
	      else{
		if(isnan(sp[nf][j])==0){
		  ...
		  //fprintf(stderr,"%d %d %g %g %g %g",nf,j,xav[1][j],sp[nf][j],rf0[nf][j],fav[2][nf]);getchar();
		}
	      }
	    }
	  }
	...
	sig/=naxes[0]/2;
	noi/=naxes[0]/2;
	fprintf(fpt,"%g %g %g %g %g\n",fav[0][nf],fav[2][nf],sig,noi,sig/noi);
	//fprintf(fpt,"%g\n",fav[0][nf]);
      }
      fclose(fpt);
      fprintf(stderr,"OK\n");
