...
{
  fitsfile *fp;
  int i,j,ii,jj,k,m,n,ns,r,rmax,cam,icen,gn,status,nfound,bitpix;
  long fpixel,naxis=2,naxes[2]={LINEPIX,LINEPIY};
  unsigned short zi[LINEPIX];
  static float zo[LINEPIY][LINEPIX],zd[LINEPIY][LINEPIX],za[NAG][LINEPIY][LINEPIX],zf[LINEPIY][LINEPIX],zb[LINEPIY/4][LINEPIX/4];
  double bg[2][LINEPIX];
  FILE *fpt;
  char fname[64],fname0[54],fname1[54],buf[256],dir[32],jstend[16],date[16],gain[16],cn[2]={'C','H'},fene[16],object[32];
  double a[NAG+1],gf,dmy,zs,z2s,zav,av,sg,bs[LINEPIX/4][4][4];
  ...
  if(argc>3){
	...
	exit(0);
      ...
      if(fits_read_key(fp, TINT, "SPEC-CTR", &icen, NULL, &status)){
  ...
  while(1){
	...
	for(j=0;j<naxes[0];j++){
      ...
      for(i=0;i<naxes[1]/4;i++){
	for(j=0;j<naxes[0]/4;j++){
	  zav=0.;ns=0;
	  for(ii=0;ii<4;ii++){
	    for(jj=0;jj<4;jj++){
	      if(isnan(zo[i*4+ii][j*4+jj])==0){
		zav+=zo[i*4+ii][j*4+jj];
		ns++;
	      }
	    }
	  }
	...
	for(ii=0;ii<4;ii++){
	  for(jj=0;jj<4;jj++){
	    ...
	    for(m=0;m<3;m++){
	      zs=0.;z2s=0.;ns=0;
	      for(i=0;i<naxes[1]/4;i++){
		if(abs(i*4+1-icen)<180) continue;
		if(isnan(zo[i*4+ii][j*4+jj]-zb[i][j])) continue;
		if(fabs(zo[i*4+ii][j*4+jj]-av)<sg*3){
		  zs+=zo[i*4+ii][j*4+jj]-zb[i][j];
		  z2s+=(zo[i*4+ii][j*4+jj]-zb[i][j])*(zo[i*4+ii][j*4+jj]-zb[i][j]);
		  ns++;
		}
	      }
	      av=zs/ns;
	      sg=sqrt(z2s/ns+av*av);
	    }
	    bs[j][ii][jj]=av;
	    //fprintf(stderr," %lf",bs[j][ii][jj]);
	  }
	  //fprintf(stderr,"\n");
	}
      }
