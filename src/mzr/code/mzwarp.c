...
int main(int argc,char *argv[])
{
  ...
  /*
  remove("tmp.fits");
  fprintf(stderr,"Write tmp.fits ... ");
  status=0;
  if (fits_create_file(&fp, "tmp.fits", &status)) printerror(status);
  if (fits_create_img(fp, bitpix, naxis, naxes, &status)) printerror(status);
  fpixel=1;
  fprintf(stderr,"%ld x %ld format ",naxes[0],naxes[1]);
  for(i=0;i<naxes[1];i++){
    if(fits_write_img(fp, TFLOAT, fpixel, naxes[0], zi[i], &status)) printerror(status);
    fpixel+=naxes[0];
  }
  if (fits_close_file(fp, &status)) printerror(status);
  fprintf(stderr,"OK\n");
  */
  ...
  for(i=4;i<naxes[1]-4;i++){
	...
	//fprintf(stderr,"%d %d: %g %g %g %g %g",nf,i,zi[i-2][j],zi[i-1][j],zi[i][j],zi[i+1][j],zi[i+2][j]);getchar();
	if(nf==1 && i-yc[0][j]>PSPEC+1){
	  fprintf(stderr,"Reset nf (%d %d: %d %g %g)\n",j,i,i,yc[0][j],i-yc[0][j]);
	  nf=0;
	}
	if(nf==NFIBER){
	  for(n=0;n<NFIBER;n++) fprintf(stderr,"%d: %g\n",n,yc[n][j]);
  ...
  */
  for(nf=0;nf<NFIBER;nf++){
    ...
    for(of=-100;of<=100;of++){
      ...
      for(n=0;n<nc;n++){
	for(nn=0;nn<nd;nn++){
	  if(fabs(cxt[nn]-cx[n][NFIBER]-of)<5.){
	    m++;
	    break;
	  }
	}
    ...
    if(nf==NFIBER/2) fprintf(stderr,"Number of detected lines: %d/%d (offset %d)\n",mmax,nc,ofmax);
    for(n=0;n<nc;n++){
      for(nn=0;nn<nd;nn++){
	if(fabs(cxt[nn]-cx[n][NFIBER]-ofmax)<5.){
	  //if(nf==NFIBER/2) fprintf(stderr,"%d: %lf %lf\n",nn,cxt[nn]+1,cxt[nn]-cx[n][NFIBER]-ofmax);
	  cx[n][nf]=cxt[nn];
	  break;
	}
      }
  ...
  if (fits_write_key(fp, TINT, "SPEC-CTR", &icen, "", &status)) printerror(status);
  tmpval=ax[0]+1;
